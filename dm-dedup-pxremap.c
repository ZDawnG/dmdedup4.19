...
struct metadata {
	 ...
	 */
	struct kvstore_pxremap_linear *kvs_linear;
	struct kvstore_pxremap_sparse *kvs_sparse;

	u8 private_data[PRIVATE_DATA_SIZE];

};

struct kvstore_pxremap_linear {
	struct kvstore ckvs;
	struct dm_array_info info;
	u64 root;
};

struct kvstore_pxremap_sparse {
	struct kvstore ckvs;
	u32 entry_size;
 ...
 * in case sparse pxremap restores last set max linear probing value
...
static struct metadata *init_meta_pxremap(void *input_param, bool *unformatted)
{
	int ret;
	//u64 smap_size, tmp;
	struct metadata *md;
	...
	struct dm_transaction_manager *tm;
	struct init_param_pxremap *p =
				(struct init_param_pxremap *)input_param;

	DMINFO("Initializing PXREMAP backend");
...
static void exit_meta_pxremap(struct metadata *md)
{
	int ret;
...
static int flush_meta_pxremap(struct metadata *md)
{
	int r;
...
static int alloc_data_block_pxremap(struct metadata *md, uint64_t *blockn)
{
	return 0;
	//return dm_sm_new_block(md->data_sm, blockn);
}

static int kvs_lookup_linear_pxremap(struct kvstore *kvs, void *key,
				   s32 ksize, void *value, int32_t *vsize);

static int kvs_insert_linear_pxremap(struct kvstore *kvs, void *key,
				   s32 ksize, void *value,
				   int32_t vsize);

static int inc_refcount_pxremap(struct metadata *md, uint64_t blockn)
{
	// if (blockn >= md->smax)
	// 	return -ERANGE;
	...
	int r = 0;
	uint64_t key = blockn, ksize = sizeof(uint64_t);
	int value = 0, vsize = 0;
	r = kvs_lookup_linear_pxremap(&md->kvs_linear->ckvs, &key, ksize, &value, &vsize);

	value += 1;
	vsize = sizeof(int);
	r = kvs_insert_linear_pxremap(&md->kvs_linear->ckvs, &key, ksize, &value, vsize);
	return r;
	//return dm_sm_inc_block(md->data_sm, blockn);
}

static int dec_refcount_pxremap(struct metadata *md, uint64_t blockn)
{
	// if (blockn >= md->smax)
	// 	return -ERANGE;
...
static int get_refcount_pxremap(struct metadata *md, uint64_t blockn)
{
	/* if (blockn >= md->smax)
		return -ERANGE;
...
static int set_refcount_pxremap(struct metadata *md, uint64_t blockn, uint32_t val)
{
	/* if (blockn >= md->smax)
		return -ERANGE;
	md->smap[blockn] = val; */
	int r = 0;
	uint64_t key = blockn, ksize = sizeof(uint64_t);
	int value = val, vsize = sizeof(int);
	r = kvs_insert_linear_pxremap(&md->kvs_linear->ckvs, &key, ksize, &value, vsize);
	return r;
}
 ...
 */
bool is_deleted_p_entry(const char *ptr, uint32_t length)
{
	int i = 0;
...
static int kvs_delete_linear_pxremap(struct kvstore *kvs,
				   void *key, int32_t ksize)
{
	/* u64 idx;
	char *ptr;
	struct kvstore_pxremap_linear *kvpxremap = NULL;

	kvpxremap = container_of(kvs, struct kvstore_pxremap_linear, ckvs);
	...
	if (idx > kvpxremap->kmax)
		return -ERANGE;

	ptr = kvpxremap->store + kvs->vsize * idx;
				   ...
				   s32 ksize, void *value, int32_t *vsize)
{
	...
	// *((int *)value) = 0;
	// return 0;
	return dm_array_get_value(&(kvpxremap->info), kvpxremap->root, idx, value);
	/* if (ksize != kvs->ksize)
		return -EINVAL;
				   ...
				   int32_t vsize)
{
	...
	kvpxremap = container_of(kvs, struct kvstore_pxremap_linear, ckvs);
	idx = *((uint64_t *)key);
	return dm_array_set_value(&(kvpxremap->info), kvpxremap->root, idx, value, &(kvpxremap->root));
	/* if (ksize != kvs->ksize)
		return -EINVAL;
...
static struct kvstore *kvs_create_linear_pxremap(struct metadata *md,
					       u32 ksize, u32 vsize,
					       u32 kmax, bool unformatted)
{
	struct kvstore_pxremap_linear *kvs;
	...
	vt.dec = NULL;
	vt.equal = NULL;
	dm_array_info_init(&(kvs->info), md->tm, &vt);
	r = dm_array_empty(&(kvs->info), &(kvs->root));
	if (r < 0) {
		kvs = ERR_PTR(r);
		goto badtree;
	}
	r = dm_array_resize(&(kvs->info), kvs->root, 0, kmax, (void*)&v, &(kvs->root));
	if (r < 0) {
		kvs = ERR_PTR(r);
		goto badtree;
	}

	flush_meta_pxremap(md);

	kvs->ckvs.kvs_insert = kvs_insert_linear_pxremap;
	kvs->ckvs.kvs_lookup = kvs_lookup_linear_pxremap;
	kvs->ckvs.kvs_delete = kvs_delete_linear_pxremap;
 ...
 * Returns 0 on success.
 */
static int kvs_delete_entry(struct kvstore_pxremap_sparse *kvpxremap,
			    ...
			    u64 cur_key_val, int ret_next)
{
	...
	if (ret_next == 0 &&
		...
		memset(cur_entry, DELETED_ENTRY, kvpxremap->entry_size);
			       __dm_bless_for_disk(&cur_key_val);

		r = dm_btree_insert(&(kvpxremap->info), kvpxremap->root,
				    &cur_key_val, cur_entry, &(kvpxremap->root));
		 ...
		 */
		r = dm_btree_remove(&(kvpxremap->info),
				    kvpxremap->root,
				    &cur_key_val,
				    &(kvpxremap->root));
...
static int kvs_delete_sparse_pxremap(struct kvstore *kvs,
				      void *key, int32_t ksize)
{
	char *cur_entry, *next_entry = NULL;
	u64 key_val, cur_key_val;
	int r = 0;
	struct kvstore_pxremap_sparse *kvpxremap = NULL;

	kvpxremap = container_of(kvs, struct kvstore_pxremap_sparse, ckvs);

	if (ksize != kvs->ksize)
		return -EINVAL;

	cur_entry = kmalloc(kvpxremap->entry_size, GFP_NOIO);
	if (!cur_entry)
		return -ENOMEM;

	key_val = (*(uint64_t *)key);

	r = dm_btree_lookup(&(kvpxremap->info), kvpxremap->root, &key_val, cur_entry);
	...
	while (r == 0) {
		...
		next_entry = kmalloc(kvpxremap->entry_size, GFP_NOIO);
		if (!next_entry)
			return -ENOMEM;

		r = dm_btree_lookup(&(kvpxremap->info),
				     kvpxremap->root,
				     &key_val, next_entry);

		if (!memcmp(cur_entry, key, ksize)) {
			/* Key found. */
			r = kvs_delete_entry(kvpxremap, cur_entry, next_entry,
					     cur_key_val, r);
 ...
 * < 0 - error on lookup
 */
static int kvs_lookup_sparse_pxremap(struct kvstore *kvs, void *key,
				      s32 ksize, void *value, int32_t *vsize)
{
	char *entry;
	u64 key_val;
	...
	entry = kmalloc(kvpxremap->entry_size, GFP_NOIO);
	if (!entry)
		return -ENOMEM;
	 ...
	 */
	for (i = 0; i <= kvpxremap->lpc_cur; i++) {
		r = dm_btree_lookup(&(kvpxremap->info), kvpxremap->root, &key_val,
		entry);
		...
		if (r == -ENODATA) {
			...
			DMWARN("kvs_lookup_sparse_pxremap: hash collision for "
 ...
 * Returns 0 on success.
 */
static int kvs_insert_sparse_pxremap(struct kvstore *kvs, void *key,
			       s32 ksize, void *value, int32_t vsize)
{
	char *entry;
	u64 key_val;
	...
	for (i = 0; i <= kvpxremap->lpc_max; i++) {
		r = dm_btree_lookup(&(kvpxremap->info), kvpxremap->root, &key_val,
		entry);
		if (r == -ENODATA ||
			is_deleted_p_entry(entry, kvpxremap->entry_size)) {
			memcpy(entry, key, ksize);
			memcpy(entry + ksize, value, vsize);
			__dm_bless_for_disk(&key_val);
			r = dm_btree_insert(&(kvpxremap->info), kvpxremap->root,
			&key_val, entry, &(kvpxremap->root));
			kfree(entry);
			if (i > kvpxremap->lpc_cur) {
				/*
	...
	"changing current max to hard limit :%d", kvpxremap->lpc_max);
	/* XXX: Need to hold lock on variable */
	kvpxremap->lpc_cur = kvpxremap->lpc_max;
	kfree(entry);
	return -ENOSPC;

}

static int kvs_iterate_sparse_pxremap(struct kvstore *kvs,
					...
					void *dc)
{
	...
	r = dm_btree_find_lowest_key(&(kvpxremap->info), kvpxremap->root, &lowest);
	if (r <= 0)
		goto out;

	r = dm_btree_find_highest_key(&(kvpxremap->info), kvpxremap->root, &highest);
	if (r <= 0)
		goto out;

	while (lowest <= highest) {
		/* Get the next entry entry in the kvs store */
		r = dm_btree_lookup_next(&(kvpxremap->info), kvpxremap->root,
		 ...
		 */
		if (r || is_deleted_p_entry(entry, kvs->ksize + kvs->vsize))
			continue;
						  ...
						  bool unformatted)
{
	...
	if (!unformatted) {
		kvs->ckvs.kvs_insert = kvs_insert_sparse_pxremap;
		kvs->ckvs.kvs_lookup = kvs_lookup_sparse_pxremap;
		kvs->ckvs.kvs_delete = kvs_delete_sparse_pxremap;
		kvs->ckvs.kvs_iterate = kvs_iterate_sparse_pxremap;
...
int get_private_data_pxremap(struct metadata *md, void **data, uint32_t size)
{
...
int set_private_data_pxremap(struct metadata *md, void *data, uint32_t size)
{
	if (size > sizeof(md->private_data))
		return -1;
...
void* get_bufio_client_pxremap(struct metadata *md)
{
	struct dm_block_manager* bm = md->meta_bm;
	return (void*)(bm->bufio);
};

struct metadata_ops metadata_ops_pxremap = {
	.init_meta = init_meta_pxremap,
	.exit_meta = exit_meta_pxremap,
	.kvs_create_linear = kvs_create_linear_pxremap,
	.kvs_create_sparse = kvs_create_sparse_pxremap,

	.alloc_data_block = alloc_data_block_pxremap,
	.inc_refcount = inc_refcount_pxremap,
	.dec_refcount = dec_refcount_pxremap,
	.get_refcount = get_refcount_pxremap,
	.set_refcount = set_refcount_pxremap,

	.flush_meta = flush_meta_pxremap,
